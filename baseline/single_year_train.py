...
import random
from baseline_utils import get_X_Y, build_path, get_git_revision_hash, compute_metrics, mask_end
...
def train_epoch(args, model, device, train_loader, county_avg, optimizer, epoch, writer=None):
    ...
    for batch_idx, (X, Y, counties, years) in enumerate(train_loader): # 397
        ...
        all_Y.append(Y)
        metrics_batch = eval(pred, Y, args) # [64, 64]
...
def train(args):
    ...
    n_train = len(X_train)
    param_setting = "{}_bs-{}_lr-{}_maxepoch-{}_sche-{}_T0-{}_testyear-{}_seed-{}".format(
        args.model, args.batch_size, args.learning_rate, args.max_epoch, args.scheduler, args.T0, args.test_year, args.seed)
