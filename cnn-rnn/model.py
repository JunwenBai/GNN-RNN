...
class CNN_RNN(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.n_m = args.num_management_vars # Original: 14, new: a lot
        self.n_extra = args.num_extra_vars + args.num_outputs # Original: 4+1, new: 6+6
        # weather
        ...
        self.lstm = nn.LSTM(input_size=80+self.n_m+self.n_extra, hidden_size=self.z_dim, num_layers=1, batch_first=True)
        self.regressor = nn.Sequential(
            nn.Linear(args.z_dim, args.z_dim//2),
            nn.ReLU(),
            nn.Linear(args.z_dim//2, args.num_outputs),
        )
    ...
    def forward(self, X, Y):
        ...
        X_soil = X[:, self.n_w:self.n_w+self.n_s].reshape(-1, 1, self.soil_depths) # [64*5*num_soil_vars, 1, soil_depths]
        ...
        X_m = X[:, self.n_w+self.n_s:self.n_w+self.n_s+self.n_m] # [64*5, n_m]
        X_extra = X[:, self.n_w+self.n_s+self.n_m:] # [64*5, n_extra]

        X_all = torch.cat((X_w, X_s, X_m, X_extra), dim=1) # [64*5, 40+40+n_m+n_extra]  TODO put X_m (progress data) back in
